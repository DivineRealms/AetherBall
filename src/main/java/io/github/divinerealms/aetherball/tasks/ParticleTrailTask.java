...
import static io.github.divinerealms.aetherball.physics.PhysicsConstants.DISTANCE_PARTICLE_THRESHOLD_SQUARED;
import static io.github.divinerealms.aetherball.physics.PhysicsConstants.GENERIC_PARTICLE_COUNT;
import static io.github.divinerealms.aetherball.physics.PhysicsConstants.GENERIC_PARTICLE_OFFSET;
import static io.github.divinerealms.aetherball.physics.PhysicsConstants.GENERIC_PARTICLE_SPEED;
import static io.github.divinerealms.aetherball.physics.PhysicsConstants.GLOW_TASK_INTERVAL_TICKS;
...
public class ParticleTrailTask extends BaseTask {
  ...
  protected void kaboom() {
    Collection<? extends Player> onlinePlayers = manager.getCachedPlayers();
    ...
    for (Player player : onlinePlayers) {
      ...
      playerSettings.put(player.getUniqueId(), manager.getPlayerSettings(player));
    }
    ...
    for (Slime cube : data.getCubes()) {
      ...
      for (Player player : onlinePlayers) {
        ...
        boolean alwaysRenderParticles = settings.isAlwaysShowParticles();
        if (!alwaysRenderParticles && distanceSquared < DISTANCE_PARTICLE_THRESHOLD_SQUARED) {
          continue;
        }
