 ...
 * <p>
 * Subclasses can use the {@link #monitoredExecution(String, Runnable)} method to wrap their event
 * handler logic with performance timing. If debug mode is enabled and the execution time exceeds
 * the configured threshold, a debug message will be broadcast to players with the appropriate
 * permission.
 * </p>
 * <p>
 * This design centralizes the performance monitoring logic that would otherwise need to be
...
public abstract class BaseListener implements Listener {
   ...
   * <p>
   * This method wraps the provided runnable in a try-finally block that measures execution time. If
   * debug mode is enabled in the settings and the execution time exceeds the configured threshold,
   * a debug message is sent to all players with the debug permission.
   * </p>
   * <p>
   * The timing is performed using {@link System#nanoTime()} for high precision, and the result is
   * converted to milliseconds for human-readable output.
   * </p>
   *
