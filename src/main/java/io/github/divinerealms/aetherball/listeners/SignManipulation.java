  ...
  public void onInteract(PlayerInteractEvent event) {
    ...
    if (block.getType() == Material.SIGN_POST || block.getType() == Material.WALL_SIGN) {
      ...
      switch (line1.toLowerCase()) {
        ...
        case "stats":
          manager.getMatchSystem().checkStats(player.getName(), player);
          break;
        ...
        case "balance":
          logger.send(player, BALANCE, String.valueOf(manager.getEconomy().getBalance(player)));
          break;

        case "highscores":
          manager.getHighscoreManager().showHighScores(player);
          break;
