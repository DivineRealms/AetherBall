 ...
 *
 * <p>Provides sign-based interfaces for joining matches, spawning cubes, checking stats, and
 * viewing highscores. Also manages cube spawn/clear buttons using colored wool blocks.
 */
public class SignManipulation extends BaseListener {
   ...
   *
   * <p>Converts shorthand [fc] signs into formatted [AetherBall] signs with colored text and
    ...
    monitoredExecution(
        () -> {
          ...
          if (event.getLine(0) != null
              && event.getLine(0).equalsIgnoreCase("[AetherBall]")
              && !player.hasPermission("aetherball.admin")
   ...
   *
   * <p>Processes sign-based commands (join queue, check stats, spawn cube, view balance/highscores)
        ...
        () -> {
            ...
            switch (color) {
              ...
              case LIME:
                ...
                Collection<Entity> nearbyEntities =
                    playerLocation.getWorld().getNearbyEntities(playerLocation, 100, 100, 100);
                int slimeCount = 0;
          ...
          if (block.getType() == Material.SIGN_POST || block.getType() == Material.WALL_SIGN) {
            ...
            if (sign.getLine(0) == null
                || !sign.getLine(0).equalsIgnoreCase("[AetherBall]")
                || !Settings.FEATURES_SIGNS.asBoolean()) {
              return;
            }
            ...
            switch (line1.toLowerCase()) {
              ...
              case "cube":
                ...
                Collection<Entity> nearbyEntities =
                    location.getWorld().getNearbyEntities(location, 100, 100, 100);
                int slimeCount = 0;
                ...
                if (slimeCount < Settings.CUBE_MAX_PER_AREA.asInt()) {
                  physicsSystem.spawnCube(player.getLocation().add(new Vector(0.5, 0.5, 0.5)));
                  physicsSystem.setButtonCooldown(player);
                  sendMessage(player, CUBE_SPAWN);
                } else {
                  sendMessage(player, ALREADY_ENOUGH_CUBES);
              ...
              case "balance":
                ...
                sendMessage(
                    player, BALANCE, String.valueOf(manager.getEconomy().getBalance(player)));
                break;
