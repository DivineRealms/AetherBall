...
public class CommandRegistry {
  ...
  private void configureMessages() {
    ...
    acf.getLocales()
        .addMessage(
            Locale.ENGLISH, MessageKeys.PERMISSION_DENIED_PARAMETER, NO_PERM_PARAMETERS.toString());
  ...
  private void registerCompletions() {
    ...
    context.registerCompletion(
        "matchtypes",
        ctx ->
            Settings.getEnabledMatchTypes().stream()
                .map(Settings::getMatchTypeName)
                .collect(Collectors.toList()));
    context.registerCompletion(
        "allmatchtypes",
        ctx ->
            Settings.getAllMatchTypeConfigs().keySet().stream()
                .map(Settings::getMatchTypeName)
                .collect(Collectors.toList()));
  }
