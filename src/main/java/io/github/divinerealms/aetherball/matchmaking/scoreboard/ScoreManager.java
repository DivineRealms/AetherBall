package io.github.divinerealms.aetherball.matchmaking.scoreboard;
...
import static io.github.divinerealms.aetherball.configs.Lang.SCOREBOARD_FOOTER;
import static io.github.divinerealms.aetherball.configs.Lang.SCOREBOARD_LINES_BLUE_PLAYERS_ENTRY;
import static io.github.divinerealms.aetherball.configs.Lang.SCOREBOARD_LINES_LOBBY;
import static io.github.divinerealms.aetherball.configs.Lang.SCOREBOARD_LINES_MATCH;
import static io.github.divinerealms.aetherball.configs.Lang.SCOREBOARD_LINES_RED_PLAYERS_ENTRY;
import static io.github.divinerealms.aetherball.configs.Lang.SCOREBOARD_LINES_WAITING_PLAYERS_ENTRY;
...
public class ScoreManager {
  ...
  public ScoreManager(Manager manager) {
    this.manager = manager;
    this.tabAPI = manager.getTabAPI();
  ...
  public void createLobbyScoreboard(Match match) {
    ...
    if (match.getLobbyScoreboard() != null) {
      try {
        scoreboardManager.removeScoreboard(match.getLobbyScoreboard());
    ...
    Scoreboard newScoreboard = scoreboardManager.createScoreboard(scoreboardName, title, lines);
  ...
  public void createMatchScoreboard(Match match) {
    ...
    if (match.getMatchScoreboard() != null) {
      try {
        scoreboardManager.removeScoreboard(match.getMatchScoreboard());
      } catch (IllegalArgumentException ignored) {
      }
    }
  ...
  public void showLobbyScoreboard(Match match, Player player) {
    ...
    if (tabPlayer != null) {
      scoreboardManager.showScoreboard(tabPlayer, match.getLobbyScoreboard());
    }
  }
  ...
  public void showMatchScoreboard(Match match, Player player) {
    ...
    TabPlayer tabPlayer = tabAPI.getPlayer(player.getUniqueId());
    if (tabPlayer != null) {
      scoreboardManager.showScoreboard(tabPlayer, match.getMatchScoreboard());
    }
  }
  ...
  private String buildLobbyScoreboard(Match match) {
    ...
    String playersList =
        playersListBuilder.length() == 0 ? NOBODY.toString() : playersListBuilder.toString();
  ...
  public void unregisterScoreboard(Scoreboard scoreboard) {
    if (tabAPI != null && manager != null && scoreboard != null) {
      scoreboardManager.removeScoreboard(scoreboard);
    }
  }
}